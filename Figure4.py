...
plt.plot(abs(freqfour), np.real(fourierQ1), label = "Q = 1")
plt.plot(abs(freqfour), np.real(fourierQ2), label = "Q = 0")
...
plt.imshow(np.transpose(fourierQ1list), extent = [0,100,0,0.3], aspect= 'auto', vmin=0)
plt.show()

plt.imshow(np.transpose(fourierQ2list), extent = [0,100,0,0.3], vmin=0, aspect='auto')
plt.show()
